...
exports.modifySauce = (req, res, next) => {
  // Si une nouvelle image est envoyée, création de l'objet sauce avec l'url de l'image
  const sauceObject = req.file
    ? {
        ...JSON.parse(req.body.sauce),
        imageUrl: `${req.protocol}://${req.get('host')}/images/${req.file.filename}`,
      }
    : // Si aucune nouvelle image est envoyée, création de l'objet sauce sans l'url de l'image
      { ...req.body };

  // Suppression de l'ancien champ userId envoyé par le client
  delete sauceObject._userId;

  // Si une nouvelle image est envoyée, suppression de l'ancienne image et mise à jour de l'objet sauce
  if (req.file) {
    // Recherche de l'objet sauce dans la BD grâce à l'ID envoyé par le client
    Sauce.findOne({ _id: req.params.id })
      .then((sauce) => {
        // Récupération du nom de l'ancienne image grâce à l'url de l'image dans l'objet sauce
        const filename = sauce.imageUrl.split('/images/')[1];
        // Suppression de l'ancienne image du dossier images
        fs.unlink(`images/${filename}`, () => {
          // Mise à jour de l'objet sauce dans la BD avec l'objet sauce modifié
  ...
  // Si aucune nouvelle image n'est envoyée, mise à jour de l'objet sauce sans supprimer l'ancienne image
  else {
    // Mise à jour de l'objet sauce dans la BD avec l'objet sauce modifié
    Sauce.updateOne({ _id: req.params.id }, { ...sauceObject, _id: req.params.id })
